...
from plugins.malik.extra import GHHMT, SMART_PIC, STTS, MQTT, MQTTT, MQTTP, PPC, REPORT, PURGE, MUTE
from pyrogram.errors.exceptions.bad_request_400 import MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty
from Script import script, ALURT_FND, M_NT_FND, M_NNT_FND, M_NNT_FNDD
import pyrogram
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, M_NT_F, AUTH_USERS, M_N_F, VIDEO_VD, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
...
@Client.on_message(filters.group & filters.pm & filters.text & ~filters.edited & filters.incoming)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            abc = await message.reply_photo(photo=imdb.get('poster'), caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            ...
            sbs = await message.reply_photo(photo=poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(300)
            await sbs.delete()
            await message.delete()
        except Exception as e:
            logger.exception(e)
            mbb = await message.reply_photo(photo=SMART_PIC, caption=cap, reply_markup=InlineKeyboardMarkup(btn)) 
            await asyncio.sleep(300)
            await mbb.delete()
            await message.delete()
    else:
        bfb = await message.reply_photo(photo=SMART_PIC, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
